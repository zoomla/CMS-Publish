<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="ZoomLaCMS.Design.SPage.Default" EnableViewState="false" ValidateRequest="false" MasterPageFile="~/Design/Master/Empty2.Master" %>
...
<nav class="navbar navbar-expand-sm">
      <div class="container">
          <div class="pull-right" style="line-height:50px;font-weight:bolder;font-size:16px;"><i class="zi zi_flag"></i> <%=pageMod.PageName %></div>
        <div class="navbar-header">
          <a class="navbar-brand" href="javascript:;" onclick="scope.showDiag('<%=CustomerPageAction.customPath2+"Template/SPage/AddPage.aspx" %>','添加新页');"><i class="zi zi_fileNew"></i> 添加新页</a>
        </div>
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
          <ul class="nav navbar-nav">
            <li><a href="javascript:;" onclick="scope.save('PageDesign.aspx?ID=<%:pageMod.ID %>');"><i class="zi zi_pencilalt"></i> 页面设计</a></li>
            <li><a href="javascript:;" onclick="scope.save('<%:string.IsNullOrEmpty(pageMod.ViewUrl)?"PreView.aspx?ID="+pageMod.ID:pageMod.ViewUrl %>');"><i class="zi zi_globe"></i> 预览页面</a></li>
            <li><a href="javascript:;" onclick="scope.save();"><i class="zi zi_save"></i> 保存布局</a></li>
          </ul>
...
<div id="editor" ng-app="APP" ng-controller="ZLCtrl" class="container">
  <div class="text_600 text-center r_gray" style="margin:0 auto;color:#999;" ng-if="Layout.list.length<1">
   ...
   <div class="layout_list">
       <div ng-repeat="layout in Layout.list|orderBy:'order' track by $index">
            <div class="layout row clearfix" ng-switch="layout.type">
                <div class="btnwrap">
                    <button type="button" class="btn btn-xs btn-info" title="删除" ng-click="Layout.suredel(layout);"><i class="zi zi_trashalt"></i></button>
                    <button type="button" class="btn btn-xs btn-info" title="上移" ng-disabled="!Layout.canmove(layout,'pre')" ng-click="Layout.move(layout,'pre')"><i class="zi zi_circleUps"></i></button>
                    <button type="button" class="btn btn-xs btn-info" title="下移" ng-disabled="!Layout.canmove(layout,'next')" ng-click="Layout.move(layout,'next')"><i class="zi zi_circleDowns"></i></button>
                    <span ng-if="layout.type=='fill'"><i class="zi desktop"></i> 全屏</span>
                </div>
                <div ng-switch-when="fill" class="col-12 column">
                ...
                <div ng-switch-when="12" class="col-12 column">
                    <div class="comp" ng-repeat="comp in layout.middle|orderBy:'order'  track by $index">
                       <input type="text" ng-model="comp.name" class="form-control text_md" />
                        <div class="pull-right" ng-init="list=layout.middle">
                            <compbtns></compbtns>
                ...
                <div ng-switch-when="48" class="row">
                ...
                <div ng-switch-when="84"  class="row">
                    <div class="col-8 column">
                        <div class="comp" ng-repeat="comp in layout.left|orderBy:'order'  track by $index">
                            <input type="text" ng-model="comp.name" class="form-control text_md" />
                            <div class="pull-right" ng-init="list=layout.left">
   ...
   <div class="text-center">
        <button type="button" class="addLayout btn btn-info" ng-click="showAddLayout();"><i class="zi zi_plus"></i> 添加新布局</button>
   </div>
</div>
</asp:Content>
...
.btn-xs {padding: .12rem .25rem;font-size: .435rem; line-height: 1.5;border-radius: .1rem; }
.row {width:100%;}
</style>
...
<script>
    ...
    angular.module("APP", []).controller("ZLCtrl", function ($scope) {
        ...
        var html ="<button type=\"button\" class=\"btn btn-xs btn-info\" ng-click=\"showAddComp(list)\" ng-if='$index==0' title='添加'><i class=\"zi zi_plus\"></i></button>"
            +"<button type=\"button\" class=\"btn btn-xs btn-info\" ng-click=\"Comp.copy(list,comp);\" title='复制'><i class=\"zi zi_copy\"></i></button>"
            +"<button type=\"button\" class=\"btn btn-xs btn-info\" ng-click=\"Comp.del(list,comp);\" title='删除'><i class=\"zi zi_trashalt\"></i></button>"
            +"<button type=\"button\" class=\"btn btn-xs btn-info\" ng-disabled=\"!Comp.canmove(comp,'pre',list)\"  title='上移' ng-click=\"Comp.move(comp,'pre',layout.middle);\"><i class=\"zi zi_circleUps\"></i></button>"
            +"<button type=\"button\" class=\"btn btn-xs btn-info\" ng-disabled=\"!Comp.canmove(comp,'next',list)\" title='下移' ng-click=\"Comp.move(comp,'next',layout.middle);\"><i class=\"zi zi_circleDowns\"></i></button>";
        return {
            restrict: 'E',
            replace: true,
