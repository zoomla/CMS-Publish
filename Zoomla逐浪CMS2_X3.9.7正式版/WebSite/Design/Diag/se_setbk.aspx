...
<div class="respane">
    <div class="lpane">
        ...
        <div class="color_div" id="color_div2" style="bottom:100px;background-color:#F7F7F7;">
            <input type="text" id="color_t" style="display:none;margin-bottom:8px;" class="form-control"/>
        </div>
        <div class="color_div" id="color_div" ondblclick="setbkcolor();">
            <span class="color_btn"><i class="zi zi_paintbrush" style="font-size:20px;"></i> 纯色背景</span>
        </div>
        <div class="up_div" onclick="pic.sel();">
            <div class="wait"><i class="zi zi_clouduploadalt" style="font-size:25px;"></i> 上传</div>
            <div class="work"><i class="zi zi_spinner zi_pulse" style="font-size:25px;"></i> 正在上传</div>
        </div>
...
.respane .lpane .color_div2{bottom:80px;border:none;}
</style>
<script>
...
$("#color_div").click(function () {
    var $color = $("#color_t");
    if ($color.is(":visible")) { $("#color_t").hide().trigger("colorpickersliders.hide"); }
    else
    {
        $color.show().trigger("colorpickersliders.show");
    }
});
$("#color_t").ColorPickerSliders({
    size: 'sm', placement: 'top', swatches: false, sliders: false, hsvpanel: true, previewformat: "hex",
    trigger: "manual",
    title: "选择背景色<a href='javascript:;' class='btn btn-info btn-xs pull-right color_btn' onclick='setbkcolor();'>确定</a>",
