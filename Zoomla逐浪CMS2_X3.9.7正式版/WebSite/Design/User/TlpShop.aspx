...
<ItemTemplate>
<li>
    <img class="tlpimg" src="<%#Eval("PreviewImg") %>" onerror="this.src=null;this.src='/UploadFiles/nopic.svg';" />
    <div class="tlp_opdiv">
        <strong class="r_gray"><%#Eval("TlpName") %></strong>
        <span class="r_gray">价格:<span class="r_red"><%#GetPrice() %></span></span>
        <div>
            <a href="/Design/Preview.aspx?TlpID=<%#Eval("ID") %>" class="btn btn-xs btn-info" target="_blank"><i class="zi zi_search"></i> 浏览</a>
            <asp:LinkButton runat="server" CssClass="btn btn-xs btn-info" CommandName="apply" CommandArgument='<%#Eval("ID") %>' OnClientClick="return confirm('确定要应用此模板吗,将会覆盖你原有配置');"><i class="zi zi_adjust"></i> 应用</asp:LinkButton>
        </div>
    </div>
</li>
</ItemTemplate>
...
<li class="more" title="更多模板" onclick="window.open('/Class_2/Default.aspx');">
    <i class="zi zi_fileNew r_gray"></i>
