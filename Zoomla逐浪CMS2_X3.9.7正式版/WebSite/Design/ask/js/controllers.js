...
.controller("AskAddCtrl", function ($scope, $ionicLoading, $state, User, Loading, Ask) {
    ...
    picup.opts.up_before = function () { Loading.show(); }
    picup.opts.up_after = function (data) { $scope.model.PreViewImg = data; $scope.$digest(); Loading.hide(); }
    $scope.add = function () {
        Loading.show();
        Ask.add($scope.model,
...
.controller("AskViewCtrl", function ($scope, $state, $stateParams, $ionicPopup, $ionicScrollDelegate, Loading, Ask, Question, Answer, DB) {
    ...
    $scope.geticon = function (qtype) {
        switch (qtype) {
            case "email":
                return "zi zi_envelope";
            case "mobile":
                return "zi zi_mobilealt";
            case "area":
                return "zi zi_home";
            case "date":
                return "zi zi_calendar";
        }
    }
    //提交问卷
