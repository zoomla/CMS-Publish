...
<div>
    ...
    <table class="table table-border  border staff_frm "  >
        ...
        <tr><td width="100" align="center" valign="top">今日计划 </td>
            <td id="log_box" colspan="2">
				...
				<div id="edit_middel_div" runat="server" style="display:none;">
				<textarea id="log_1" name="log_1" cols="60" rows="6" runat="server"></textarea><br />
                </div>
				...
				<div class="input-group mb-3">
				  <div class="input-group-prepend">
				   ...
				   <asp:TextBox ID="TextDate" onclick="WdatePicker({ dateFmt: 'yyyy/MM/dd HH:mm:ss' });" class="max20rem" runat="server" />
				  <div class="input-group-append">
					<asp:Button ID="Button1" CssClass="btn btn-outline-info" OnClick="Button1_Click" runat="server" Text="确定" />
                    <input id="btn1Cln" type="button" class="btn btn-outline-info" onclick="ShowHidDiv('mylog_title_div', 'edit_middel_div')" value="取消" />
				  </div>
				</div>
        ...
        <%--<asp:LinkButton ID="lbtDate" CommandArgument='<%#Eval("ID") %>' CommandName="SetDate" Text="延期到" runat="server" ></asp:LinkButton>--%>
                                ...
                                <li id="Dayshow_<%#Eval("ID") %>" style="display:none;"  class="bgyellow">
                                    ...
                                    <asp:Button ID="btnEdDay" CssClass="btn btn-outline-infp" CommandArgument='<%#Eval("ID") %>' CommandName="Edit"   runat="server" Text="确定" />
                                    <input id="Button5" type="button" class="btn btn-outline-info" onclick="ShowDiv('Dayshow_<%#Eval("ID") %>')"  value="取消" />
                                </li>
                                <li id="DayChang_<%#Eval("ID") %>" style="display:none;"  class="bgyellow">
                                    ...
                                    <asp:Button ID="Button4" CssClass="btn btn-outline-info" CommandArgument='<%#Eval("ID") %>' CommandName="EditDate"   runat="server" Text="确定" />
                                    <input id="Button6" type="button" class="btn btn-outline-info" onclick="ShowDiv('DayChang_<%#Eval("ID") %>')" value="取消" />
                                </li>
                            </ItemTemplate>
                        </asp:Repeater>
                    </ul>
        ...
        <tr><td align="center" valign="top">工作小结 </td>
            <td id="end_box"  colspan="2">
                ...
                <div id="Div1"  style="display:none;">
                    ...
                    <asp:Button ID="Button2" CssClass="btn btn-outline-info" OnClick="Button2_Click" runat="server" Text="确定" />
                    <input id="Button5" type="button" class="btn btn-outline-info" onclick="ShowHidDiv('jtit', 'Div1')" value="取消" />
        </div>
        <div id="DaiJie">
        <ul>
        ...
        <asp:Button ID="btnEdDay" CssClass="C_inputs btn btn-outline-info" CommandArgument='<%#Eval("ID") %>' CommandName="Edit"   runat="server" Text="确定" />
        <input id="Button5" type="button" class="C_inputs btn btn-outline-info" onclick="ShowDiv('Dayshow_<%#Eval("ID") %>')"  value="取消" />
        </li>
        <li id='DayChang_<%#Eval("ID") %>' style="display:none;">
        <input id='edtDate_<%#Eval("ID") %>' name='edtDate_<%#Eval("ID") %>'  value="<%#Eval("createtime") %>"  onclick="WdatePicker({ dateFmt: 'yyyy/MM/dd HH:mm:ss' });" class="M_input"  />
        <asp:Button ID="Button4" CssClass="C_inputs btn btn-outline-info" CommandArgument='<%#Eval("ID") %>' CommandName="EditDate"   runat="server" Text="确定" />
        <input id="Button6" type="button" class="C_inputs btn btn-outline-info" onclick="ShowDiv('DayChang_<%#Eval("ID") %>" value="取消" />
        </li>
        </ItemTemplate>
        </asp:Repeater>
        </ul></div></td></tr>
        ...
        <asp:Button ID="Button3" OnClick="Button3_Click" CssClass="btn btn-outline-info" runat="server" Text="确定" />
        </td></tr>
    </table>   
	</div>
</div>
