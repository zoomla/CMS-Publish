...
<start>
  <div id="ttitle_div" class="timeline_title" runat="server">
      ...
      <div class="link_div">
          ...
          <a href="javascript:;" onclick="ChatShow('<%=GetUser().UserID %>', '<%=GetUser().UserName %>');" title="发私信"><i class="zi zi_commentdots"></i></a>
          <a href="TimeLineToMHT.aspx?uid=<%=GetUser().UserID %>&type=mht" title="导出为MHT" ><i class="zi zi_fileCode"></i></a>
          <a href="TimeLineToMHT.aspx?uid=<%=GetUser().UserID %>&type=pdf" title="导出为PDF" ><i class="zi zi_fileEdit"></i></a>
      </div>
  </div>
  ...
  <asp:Repeater runat="server" ID="MsgRepeater" OnItemDataBound="MsgRepeater_ItemDataBound">
    ...
    <div class="time-timeline-block">
		...
		<div class="time-timeline-img text-center">
            <i class="zi <%#Eval("timeline").Equals("date")? "zi_home":"zi_circle" %>"></i>
		</div>
