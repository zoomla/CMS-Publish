...
public class mobile : API_Base, IHttpHandler
{
        ...
        try
        {
            switch (Action.ToLower())
            {
                ...
                case "sendvailmsg"://手机发送验证(用于手机流程注册)(Register_Mobile)
                    {
                        if (!VerifyHelper.Check(Req("hcode"))) { context.Response.Write("验证码不正确"); return; }
                        ...
                        if (mobileBll.CheckMobile(mobile, context.Request.UserHostAddress))
                        {
                            ...
                            CommonReturn ret = SMS_Helper.SendVCode(mobile,vaildnum,SMSConfig.Instance.Tlp_Reg);
                            //添加一条发送手机短信记录
                            mobileBll.Insert(new M_Safe_Mobile() { Phone = mobile, IP = IPScaner.GetUserIP(), VCode = vaildnum, Source = "register", SysRemind = ret.err });
                            retMod.retcode = M_APIResult.Success;
                        }
                        else
                        {
    ...
    private bool SendCode(string mobile, out string err)
    {
        ...
        if (mobileBll.CheckMobile(mobile, IPScaner.GetUserIP()))
        {
            ...
            CommonReturn ret= SMS_Helper.SendVCode(mobile, code,SMSConfig.Instance.Tlp_Reg);
            //添加一条发送手机短信记录
            mobileBll.Insert(new M_Safe_Mobile() { Phone = mobile, IP = IPScaner.GetUserIP(), VCode = code, SysRemind = ret.err });
