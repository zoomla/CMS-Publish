<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SuitPro.aspx.cs" Inherits="ZoomLaCMS.BU.UserShop.SuitPro" MasterPageFile="~/Common/Master/User.Master"%>
...
<%--            <asp:TemplateField HeaderText="<%$Resources:L,类型 %>">
            ...
            <asp:TemplateField HeaderText="<%$Resources:L,操作 %>" ItemStyle-CssClass="codetr">
                <ItemTemplate>
                    <a class="option_style" href="<%#GetShopUrl() %>" target="_blank" title="<%=Resources.L.预览 %>"><i class="zi zi_eye"></i></a>
                    <a class="option_style" href="SuitProAdd.aspx?id=<%#Eval("id")%>"><i class="zi zi_pencilalt" title="<%=Resources.L.编辑 %>"></i></a>
                    <asp:LinkButton runat="server" CssClass="option_style" CommandName="del" CommandArgument='<%# Eval("id") %>' OnClientClick="return confirm('确定要将商品移入回收站吗');"><i class="zi zi_trashalt" title="<%=Resources.L.删除 %>"></i><%=Resources.L.删除 %></asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>
