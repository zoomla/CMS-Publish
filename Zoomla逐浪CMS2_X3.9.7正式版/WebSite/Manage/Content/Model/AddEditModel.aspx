...
<title><%=Resources.L.编辑模型 %></title>
</asp:Content>
<asp:Content ContentPlaceHolderID="Content" Runat="Server">
<%=Call.SetBread( new Bread[] {
            new Bread("/{manage}/I/Main.aspx",Resources.L.工作台),
            new Bread("ModelManage.aspx?ModelType="+ModelType,Resources.L.模型管理),
...
<div class="row sysRow list_choice table-responsive-md">
	...
	<tr>
		<th scope="col" class="w12rem_lg" >
			<strong><%=bll.GetModelType(ModelType) %><%=Resources.L.名称 %></strong>
		</th>
		<td scope="col">
			...
			<asp:RequiredFieldValidator ID="RV1" runat="server" ErrorMessage="RequiredFieldValidator" ControlToValidate="TxtModelName">
			    <small class="text-muted"><%=Resources.L.模型名称不能为空 %></small>
			</asp:RequiredFieldValidator></span>
        </td>
	</tr>
	<tr>
		<th>
			<strong><%=Resources.L.创建的数据表名 %></strong>
		</th>
		<td>
            ...
            <span id="checkname_span" style="color:red;display:none;"><%=Resources.L.表名重复 %></span>
            <asp:RequiredFieldValidator ID="RequiredFieldValidator4" runat="server" ErrorMessage="RequiredFieldValidator" ControlToValidate="TxtTableName"><small class="text-muted"><%=Resources.L.数据表名不能为空 %></small></asp:RequiredFieldValidator>
		</td>
	</tr>
	...
	<tr>
		<th>
			<strong><%=Resources.L.项目名称 %> <span class="text-danger">*</span></strong>
		</th>
		<td>
			...
			<small class="text-muted"><%=Resources.L.项目名称不能为空 %></small>
			</asp:RequiredFieldValidator></span>
		</td>
	</tr>
	<tr>
		<th>
			<strong><%=Resources.L.项目单位 %> <span class="text-danger">*</span></strong>
		</th>
		<td>
			<div class="input-group">
			<asp:TextBox ID="TxtItemUnit" runat="server" Text="<%$Resources:L,篇 %>" CssClass="form-control max20rem" MaxLength="20" />
			...
			<small class="text-muted"><%=Resources.L.项目单位不能为空 %></small>
			</asp:RequiredFieldValidator></span>
		</td>
	</tr>
   <tr>
		<th>
			<strong><%=Resources.L.是否复制 %></strong>
			  <br />
			  <%=Resources.L.确定模型是否可以复制 %>
		</th>
		<td>
    ...
    <tr>
		<th>
			<strong><%=Resources.L.是否批量添加 %></strong>
			  <br />
			  <%=Resources.L.是否可以批量添加 %>
		</th>
		<td>
	...
	<tr>
		<th>
			<strong><%=Resources.L.项目图标 %></strong>
		</th>
		<td>
			<div class="input-group">
	...
	<tr>
		<th>
			<strong><%=Resources.L.模型描述 %></strong>
		</th>
		<td>
	...
	<tr>
		...
		<td colspan="2" class="text-left panel_footers">
			<asp:Button ID="EBtnSubmit"  Text="<%$Resources:L,保存信息 %>" OnClientClick="return CheckData();" CssClass="btn btn-info"  OnClick="EBtnSubmit_Click" runat="server" />
			&nbsp;&nbsp;
            <a href="ModelManage.aspx?ModelType=<%=Request["ModelType"] %>" class="btn btn-outline-info"><%=Resources.L.返回列表 %></a>
		</td>
	</tr>
</table>
...
function SelUnit(){
    var units= "<%=Resources.L.篇%> ,<%=Resources.L.个%> ,<%=Resources.L.张%> ,<%=Resources.L.件%> ";
...
function SelItem(){
    var units= "<%=Resources.L.文章%>,<%=Resources.L.软件%>,<%=Resources.L.图片%>,<%=Resources.L.商品%>";
    var array=units.split(",");
    for (var i = 0; i < array.length; i++) {
        var str="<button type='button' class='btn btn-outline-info'>"+array[i]+"</button>"
        $("#ItemTd").append(str);
