...
<title><%=Resources.L.评论管理 %></title>
<%=Call.SetBread(new Bread[] {
	new Bread("/{manage}/Main.aspx",Resources.L.工作台),
	new Bread("ContentManage.aspx",Resources.L.内容管理),
	new Bread() {url="/{manage}/Content/CommentManage.aspx", text=Resources.L.评论管理,addon="" }},
	Call.GetHelp(58)
	)
%>
</asp:Content>
...
<div class="list_choice">
	...
	<ZL:ExGridView ID="Egv" runat="server" AutoGenerateColumns="False" DataKeyNames="CommentID" PageSize="10"
		 ...
		 AllowPaging="True" AllowSorting="True" CssClass="table table-striped table-bordered" EnableTheming="False" EnableModelValidation="True" EmptyDataText="<%$Resources:L,没有内容 %>">
		<Columns>
			...
			<asp:TemplateField HeaderText="操作">
				<ItemTemplate>
					...
					<asp:LinkButton CommandName="Audit" CommandArgument='<%# Eval("CommentID") %>' runat="server" CssClass="option_style">审核</asp:LinkButton>
				</ItemTemplate>
			</asp:TemplateField>
		</Columns>
	...
	<div class="panel panel-footer" >
		<asp:Button ID="Button1" runat="server" Text="<%$Resources:L,删除评论 %>" OnClick="BtnSubmit1_Click" CssClass="btn btn-outline-danger" OnClientClick="if(!IsSelectedId()){alert('<%=Resources.L.请选择操作项 %>');return false;}" />
		<asp:Button ID="Button2"   runat="server" Text="<%$Resources:L,审核通过 %>" OnClick="BtnSubmit2_Click" OnClientClick="if(!IsSelectedId()){alert('<%=Resources.L.请选择操作项 %>');return false;}" CssClass="btn btn-outline-info" />
		<asp:Button ID="Button3"  runat="server" Text="<%$Resources:L,取消审核 %>" OnClick="BtnSubmit3_Click" OnClientClick="if(!IsSelectedId()){alert('<%=Resources.L.请选择操作项 %>');return false;}" CssClass="btn btn-outline-info" />
		</div>
		</div>
</div>
