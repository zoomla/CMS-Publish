		...
		<tr ondblclick="location='ShowContent.aspx?GID=<%#Eval("GeneralID") %>';" data-order="<%#Eval("OrderID") %>" data-gid="<%#Eval("GeneralID") %>">
			...
			<td style="position:relative;">
                ...
                <asp:LinkButton runat="server" ID="lbDelete" CssClass="option_style" Enabled='<%#getenabled(Eval("GeneralID").ToString()) %>' CommandName="Del" 
                    CommandArgument='<%# Eval("GeneralID") %>' OnClientClick="return confirm('<%=Resources.L.你确定将该数据删除到回收站吗 %>?');"><i class="zi zi_trashalt" title="删除"></i></asp:LinkButton>
                <a class="option_style" href="/Item/<%#Eval("GeneralID") %>.aspx" target="_blank"><i class="zi zi_globe" zico="全球"></i><%=Resources.L.浏览 %></a>
...
<div>
	<asp:Button ID="btnDeleteAll" CausesValidation="false" runat="server" Text="<%$Resources:L,删除 %>" OnClick="btnDeleteAll_Click" OnClientClick="if(!IsSelectedId()){alert('<%=Resources.L.请选择删除项 %>');return false;}else{return confirm('<%=Resources.L.你确定要将所有选择项放入回收站吗 %>？')}" CssClass="btn btn-info" />
	...
	<asp:Button ID="Button2" CausesValidation="false" runat="server"
		Text="<%$Resources:L,推荐 %>" OnClientClick="if(!IsSelectedId()){alert('<%=Resources.L.请选择项 %>');return false;}else{return confirm('<%=Resources.L.你确定要将所有选择项设为推荐吗 %>？')}" CssClass="btn btn-info" OnClick="Button2_Click" />
	<asp:Button ID="Button3" CausesValidation="false" runat="server"
		Text="<%$Resources:L,取消推荐 %>" OnClientClick="if(!IsSelectedId()){alert('<%=Resources.L.请选择项 %>');return false;}else{return confirm('<%=Resources.L.你确定要取消所有选择项推荐吗 %>？')}" CssClass="btn btn-info" OnClick="Button3_Click" />
	<asp:Button ID="btnMove" CausesValidation="false" runat="server"
		Text="<%$Resources:L,移动 %>" OnClick="btnMove_Click" OnClientClick="if(!IsSelectedId()){alert('<%=Resources.L.请选择移动项 %>');return false;}else{return true}" CssClass="btn btn-info" />
	<asp:Button ID="btnAddToSpecial" CausesValidation="false" runat="server"
		Text="<%$Resources:L,添加到专题 %>" OnClick="btnAddToSpecial_Click" OnClientClick="if(!IsSelectedId()){alert('<%=Resources.L.请选择添加到专题的内容 %>');return false;}else{return true}" CssClass="btn btn-info" />
	<asp:Button ID="Button1" CausesValidation="false" runat="server"
		Text="<%$Resources:L,批量修改 %>" CssClass="btn btn-info" OnClientClick="if(!IsSelectedId()){alert('<%=Resources.L.请选择修改项 %>');return false;}else{return confirm('<%=Resources.L.你确定要修改选中内容吗 %>？')}"
	...
	<asp:Button ID="btnAudit" CausesValidation="false" runat="server"
		Text="<%$Resources:L,审核通过 %>" CssClass="btn btn-info" OnClick="btnAudit_Click" OnClientClick="if(!IsSelectedId()){alert('<%=Resources.L.请选择审核项 %>');return false;}else{return confirm('<%=Resources.L.你确定要审核选中内容吗 %>？')}" />
	<asp:Button ID="btnUnAudit" CausesValidation="false" runat="server"
		Text="<%$Resources:L,取消审核 %>" CssClass="btn btn-info" OnClick="btnUnAudit_Click" OnClientClick="if(!IsSelectedId()){alert('<%=Resources.L.请选择审核项 %>');return false;}else{return confirm('<%=Resources.L.你确定要取消审核选中内容吗 %>？')}" />
    <input type="button" class="btn btn-info" onclick="if(!IsSelectedId()){alert('<%=Resources.L.请选择内容 %>');return false;}else{ShowSelNode();}" value="推送" />
    <asp:Button ID="ExportExcel_B" runat="server" Text="<%$Resources:L,Excel导出 %>" CssClass="btn btn-info" OnClick="ExportExcel_B_Click" />
...
<div visible="false" runat="server" id="audit_Div" class="margin_t10">
	<asp:Button runat="server" CssClass="btn btn-success" ID="NextStep_Btn" OnClick="NextStep_Btn_Click" Text="<%$Resources:L,工作流批量审批 %>" OnClientClick="return confirm('<%=Resources.L.确定要批量审批吗 %>？');" />
	<asp:Button runat="server" CssClass="btn btn-success" ID="StepReject_Btn" OnClick="StepReject_Btn_Click" Text="<%$Resources:L,工作流批量拒绝 %>" OnClientClick="return confirm('<%=Resources.L.确定要批量拒绝吗 %>？');" />
</div>
<div id="timeReDiv" style="display:none;" class="margin_t10">
	<table class="table table-bordered">
		...
		<tr>
			...
			<td>
				<asp:Button runat="server" ID="timeReConfBtn" OnClick="timeReConfBtn_Click" OnClientClick="return scheCheck();" CssClass="btn btn-group" Text="<%$Resources:L,创建任务 %>" CausesValidation="false" />
...
<script>
    ...
    function scheCheck() {
        ...
        var now = new Date();
        if (time < now) { alert("<%=Resources.L.执行时间无效 %>"); return false; }
        var len = $("input[name=idchk]:checked").length;
        if (len < 1) { alert("<%=Resources.L.未选择文章 %>"); return false; }
        return true;
    }
	$(function () {
	...
	function ShowSelNode()
	{
	    ...
	    ShowComDiag("/Common/NodeList.aspx?Source=content", "<%=Resources.L.请选择需推送的节点 %><input type='button' value='确定' onclick='SurePush();' class='btn btn-primary'>");
	}
	function SurePush()
	{ 
	...
	function PageCallBack(action, vals, pval) 
	{
	    ...
	    $("#pushcon_hid").val(ids);
	    ids=ids.replace(/ /g,"");
	    if(ids==""){alert("<%=Resources.L.未选择需要推送的节点 %>");return false;}
	    else{
	        $("#PushCon_Btn").click();
	        var mask=new ZL_Dialog();
	        mask.ShowMask("<%=Resources.L.正在推送文章中 %>");
	    }
	}
</script>
</asp:Content>