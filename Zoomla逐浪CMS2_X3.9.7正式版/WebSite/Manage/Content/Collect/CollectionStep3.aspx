...
<asp:Content runat="server" ContentPlaceHolderID="head"><title><%=Resources.L.采集项目 %></title></asp:Content>
<asp:Content runat="server" ContentPlaceHolderID="Content">
    <table class="table table-bordered table-striped" style="margin-bottom:30px;">
        <tr><td colspan="4" class="text-center"><strong><%=Resources.L.系统字段 %></strong></td></tr>
        <tr><td class="td_l"><%=Resources.L.字段别名 %></td><td class="td_m"><%=Resources.L.字段名称 %></td><td class="td_m"><%=Resources.L.字段类型 %></td><td><%=Resources.L.赋值规则 %></td></tr>
        <asp:Repeater runat="server" ID="SYS_RPT">
            <ItemTemplate>
                <tr id="item_<%#Eval("FieldName") %>" data-field="<%#Eval("FieldName") %>" class="item_field">
                    <td><%#Eval("FieldAlias") %></td>
                    <td><%#Eval("FieldName") %></td>
                    <td><%#Eval("FieldType") %></td>
                    <td>
                    <label><input type="radio" name="<%#Eval("FieldName")+"_rad" %>" class="colltype_rad" value="1" checked="checked"/><%=Resources.L.使用默认值 %></label>
                    <label><input type="radio" name="<%#Eval("FieldName")+"_rad" %>" class="colltype_rad" value="2" /><%=Resources.L.指定值 %> <input type="text" class="form-control text_300 collvalue_t"/></label>
    ...
    <table class="table table-bordered table-striped">
        <tr><td colspan="4" class="text-center"><strong><%=Resources.L.扩展字段 %></strong></td></tr>
        <tr><td class="td_l"><%=Resources.L.字段别名 %></td><td class="td_m"><%=Resources.L.字段名称 %></td><td class="td_m"><%=Resources.L.字段类型 %></td><td><%=Resources.L.赋值规则 %></td></tr>
    ...
    <div class="text-center Conent_fix">
        <div class="btn-group cmds pull-left margin_l5">
            <span class="btn btn-info"><i class="zi zi_comment"></i> <%=Resources.L.指令 %></span>
            <button type="button" class="btn btn-default cmd">{<%=Resources.L.当前管理员名 %>}</button>
            <button type="button" class="btn btn-default cmd">{<%=Resources.L.当前日期 %>}</button>
        </div>
        <a href="CollectionStep2.aspx?itemid=<%:ItemID %>" class="btn btn-primary"><%=Resources.L.上一步 %></a>
