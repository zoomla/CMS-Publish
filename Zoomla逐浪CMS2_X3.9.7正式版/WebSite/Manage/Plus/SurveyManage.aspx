    ...
    <ZL:ExGridView ID="EGV"  CssClass="table table-striped table-bordered margin_t10" GridLines="None" runat="server" AutoGenerateColumns="False" AllowPaging="True" PageSize="10"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="操作" ItemStyle-Width="350">
                <ItemTemplate>
					<a href="Survey.aspx?ID=<%# Eval("ID") %>"> <i class="zizi_pencilalt" title="修改"></i></a>
                    <a href="/Vote/Index?id=<%#Eval("ID") %>" target="_blank" > <i class="zi zi_globe"></i>查看</a>
                    <a href="SurveyItemList.aspx?Sid=<%#Eval("ID") %>" > <i class="zi zi_listalt"></i>问题列表</a>
                    <a href="SurveyResult.aspx?id=<%#Eval("ID") %>" > <i class="zi zi_chartpie" title="分析"></i>分析</a>
                    <a href="SurveyResultList.aspx?ID=<%#Eval("ID") %>" > <i class="zi zi_list"></i>结果列表</a>
                    
                    <asp:LinkButton ID="lbtnDel" runat="server" CommandName="Del" CommandArgument='<%# Eval("ID") %>' OnClientClick="return confirm('确实要删除吗？');" ><i class="zi zi_trashalt" title="删除"></i></asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>
