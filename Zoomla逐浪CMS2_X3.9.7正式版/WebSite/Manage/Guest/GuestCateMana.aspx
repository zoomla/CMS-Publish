...
<asp:Content ContentPlaceHolderID="head" Runat="Server"><title><%=Resources.L.栏目管理 %></title></asp:Content>
<asp:Content  ContentPlaceHolderID="Content" Runat="Server">
	<%=Call.SetBread(new Bread[] {
		new Bread("/{manage}/Main.aspx",Resources.L.工作台),
        new Bread("/{manage}/Guest/GuestCateMana.aspx",Resources.L.栏目管理),
        new Bread() {url="", text="<a href='javascript:;' onclick='ShowCate(0,0);' id='showDiv'>["+Resources.L.添加版面目 +"]</a> <a href='javascript:ShowOrder(0);'> ["+Resources.L.排序管理 +"]</a>",addon="" }},
		Call.GetHelp(92)
		)
    %>
    ...
    <asp:Repeater runat="server" ID="RPT">
        <HeaderTemplate>
            <table id="EGV"  class="table table-bordered table-hover list_choice listEGV">
             <tr><th class="w1rem"><input type="checkbox" id="ALLID_Chk"/></th><th>ID</th><th><%=Resources.L.图片 %></th><th><%=Resources.L.栏目名称 %></th><th><%=Resources.L.栏目状态 %></th><th><%=Resources.L.访问权限 %></th><th><%=Resources.L.计数 %></th><th><%=Resources.L.操作 %></th></tr>
        </HeaderTemplate>
        <ItemTemplate>
            <tr data-layer="1" data-id="<%#Eval("CateID") %>" data-pid="0"  onclick="ShowChild(this,<%#Eval("CateID") %>);" ondblclick="ToggleChild(this);">
                ...
                <td class='optd'>
                    <a href="javascript:;" onclick="ShowCate(<%#Eval("CateID") %>,0);"><i class="zi zi_pencilalt" title="<%=Resources.L.修改 %>"></i></a>
                    <a href="javascript:;" onclick="ShowCate(0,<%#Eval("CateID")%>);"><i class='zi zi_plus' title='<%=Resources.L.添加 %>'></i><%=Resources.L.版面 %></a>
                    <a href='BarAuthSet.aspx?ID=<%#Eval("CateID")%>'><i class='zi zi_key' title='<%=Resources.L.权限 %>'></i><%=Resources.L.权限 %></a>
                    <%#Convert.ToInt32(Eval("ChildCount"))>0?"<a href=\"javascript:ShowOrder('"+Eval("Cateid")+"');\"><i class='zi zi_listol' title='排序'></i>排序</a>":"" %>
                    <a href="javascript:;" onclick="DelConfirm(<%#Eval("CateID") %>,1)"><i class="zi zi_filealt" title="<%=Resources.L.清空 %>"></i><%=Resources.L.清空本版数据 %></a>
                    <a href="javascript:;" onclick='DelConfirm(<%#Eval("CateID") %>,0);'><i class="zi zi_trashalt" title="<%=Resources.L.删除 %>"></i><%=Resources.L.删除 %></a>
                </td>
            </tr>
        </ItemTemplate>
        <FooterTemplate>
	...
	<tr>
		<td>
            <asp:Button ID="Dels_Btn" runat="server" CssClass="btn btn-outline-danger" Text="<%$Resources:L,批量删除 %>" OnClientClick="return confirm('是否确认删除选中栏目?')" OnClick="Dels_Btn_Click" />                 
            <asp:Button ID="BtnSetRecomPosation" runat="server" CssClass="btn btn-info" Text="<%$Resources:L,批量推荐 %>" OnClick="BtnSetRecomPosation_Click" />
            <asp:Button ID="BtnSetChanlnePosation" runat="server" CssClass="btn btn-outline-danger" Text="<%$Resources:L,取消推荐 %>" OnClick="BtnSetChanlnePosation_Click" />
		</td>
	</tr>                
</table>
