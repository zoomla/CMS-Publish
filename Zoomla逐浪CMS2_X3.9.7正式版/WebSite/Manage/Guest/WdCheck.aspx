...
<div class="list_choice">
    <ol class="breadcrumb fixed-top">
        ...
        <li class="breadcrumb-item"><%=Resources.L.问答管理 %></li>
        ...
        <div id="sel_box" runat="server">
            <div class="input-group pull-left max20rem">
                <asp:TextBox runat="server" ID="Key_T" class="form-control" placeholder="<%$Resources:L,请输入问题内容 %>" />
    ...
    <div class="borders">
        <ul class="nav nav-tabs">
            <li class="nav-item" data-index="-100"><a class="nav-link" onclick="ShowTabss(-100)"><%=Resources.L.所有问答 %></a></li>
            <li class="nav-item" data-index="0"><a class="nav-link" onclick="ShowTabss(0)"><%=Resources.L.待审核 %></a></li>
            <li class="nav-item" data-index="1"><a class="nav-link" onclick="ShowTabss(1)"><%=Resources.L.已审核 %></a></li>
            <li class="nav-item" data-index="2"><a class="nav-link" onclick="ShowTabss(2)"><%=Resources.L.新版本待审 %></a></li>
        </ul>
    </div>
    ...
    <div class="panel panel-default" >
        <div class="panel panel-body">
            <ZL:ExGridView ID="Egv" runat="server" AutoGenerateColumns="False" DataKeyNames="ID" PageSize="10" 
                ...
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="<%$Resources:L,问题内容 %>">
                        <ItemTemplate>
                            <a href="Asklist.aspx?ID=<%# Eval("ID") %>"><%#GetContent(Eval("Qcontent") as string) %></a>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="<%$Resources:L,问题类型 %>" ItemStyle-CssClass="td_m">
                        <ItemTemplate>
                            <a href="WdCheck.aspx?QueType=<%#Eval("QueType") %>"><%#gettype(Eval("QueType").ToString())%></a>
                        </ItemTemplate>
                        <HeaderStyle Width="12%"></HeaderStyle>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="<%$Resources:L,提交人 %>" ItemStyle-CssClass="td_m">
                        <ItemTemplate>
                            <a href="javascript:;" onclick="user.showuinfo('<%#Eval("UserId") %>');"><%#Eval("UserName")%></a>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="<%$Resources:L,提交时间 %>" ItemStyle-CssClass="td_l">
                        <ItemTemplate>
                            <%#Eval("AddTime")%>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="<%$Resources:L,推荐 %>" ItemStyle-CssClass="td_m">
                        <ItemTemplate>
                            <%#gettj(Eval("Elite").ToString())%>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="<%$Resources:L,审核 %>" ItemStyle-CssClass="td_m">
                        <ItemTemplate>
                    ...
                    <asp:TemplateField HeaderText="<%$Resources:L,操作 %>" HeaderStyle-Width="240">
                        <ItemTemplate>
                            <a href="/Ask/SearchDetails?soure=manager&ID=<%#Eval("ID") %>" target="_blank"><i class="zi zi_eye" title="<%=Resources.L.预览 %>"></i></a>
                            ...
                            <a href="javascript:;" onclick="SelUser(<%#Eval("ID") %>)"><i class="zi zi_fly" title="<%=Resources.L.推送 %>"></i><%=Resources.L.推送 %></a>
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
        ...
        <div class="panel panel-footer">
            <asp:Button ID="Button1" runat="server" Text="<%$Resources:L,批量删除 %>" OnClick="BtnSubmit1_Click" UseSubmitBehavior="False" OnClientClick="if(!confirm('确定要批量删除吗？')){return false;}" CssClass="btn btn-outline-danger" />
            <asp:Button ID="Button2" runat="server" Text="<%$Resources:L,审核通过 %>" OnClick="BtnSubmit2_Click" UseSubmitBehavior="False" CssClass="btn btn-outline-info" />
            <asp:Button ID="Button3" runat="server" Text="<%$Resources:L,取消审核 %>" OnClick="BtnSubmit3_Click" UseSubmitBehavior="False" CssClass="btn btn-outline-danger" />
            <asp:Button ID="Button4" runat="server" Text="<%$Resources:L,批量推荐 %>" OnClick="BtnSubmit4_Click" UseSubmitBehavior="False" CssClass="btn btn-outline-info" />
            <asp:Button ID="Button5" runat="server" Text="<%$Resources:L,取消推荐 %>" OnClick="BtnSubmit5_Click" UseSubmitBehavior="False" CssClass="btn btn-outline-danger" />
            <button type="button" class="btn btn-info" onclick="SelUser(0)"><%=Resources.L.批量推送 %></button>
