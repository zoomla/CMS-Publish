...
<asp:Content runat="server" ContentPlaceHolderID="head"><title><%=Resources.L.组合商品 %></title></asp:Content>
...
<div>
    <ol class="breadcrumb navbar-fixed-top">
        ...
        <li class='breadcrumb-item'><%=Resources.L.组合商品 %> [<a href='GroupProAdd.aspx'><%=Resources.L.添加组合 %></a>]</li>
...
<ZL:ExGridView ID="EGV" runat="server" AutoGenerateColumns="False" PageSize="20" IsHoldState="false" 
    ...
    <Columns>
        ...
        <asp:BoundField HeaderText="<%$Resources:L,组合名称 %>" DataField="Name" />
        <asp:BoundField DataField="CDate" DataFormatString="{0:yyyy年MM月dd日}" HeaderText="<%$Resources:L,创建时间 %>" />
        <asp:BoundField DataField="Remind" HeaderText="<%$Resources:L,备注 %>" />
        <asp:TemplateField HeaderText="<%$Resources:L,操作 %>" >
            <ItemTemplate>
                <a href="GroupProAdd.aspx?id=<%#Eval("ID") %>"><i class="zi zi_pencilalt" title="<%=Resources.L.修改 %>"></i></a>
...
<div class="alert alert-light">
    <asp:Button runat="server" ID="BatDel_Btn" Text="<%$Resources:L,批量删除 %>" OnClick="BatDel_Btn_Click" OnClientClick="return confirm('确定要删除吗');" class="btn btn-outline-danger" />
</div>
</asp:Content>
