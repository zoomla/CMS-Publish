...
<asp:Content runat="server" ContentPlaceHolderID="head">
    <title><%=Resources.L.编辑管理员 %></title>
</asp:Content>
<asp:Content runat="server" ContentPlaceHolderID="Content">
<%=Call.SetBread(new Bread[] {
		new Bread("/{manage}/Main.aspx",Resources.L.工作台),
        new Bread("UserManage.aspx",Resources.L.用户管理),
		new Bread("AdminManage.aspx",Resources.L.管理员管理),
        new Bread() {url="", text=Resources.L.编辑管理员,addon="" }}
		)
    %>
 	<div class="list_choice">
    <!-- Nav tabs -->
    <ul class="nav nav-tabs" role="tablist">
        <li class="nav-item"><a class="nav-link active" href="#home" role="tab" data-toggle="tab"><%=Resources.L.基本信息 %></a></li>
        <li class="nav-item"><a class="nav-link" href="#profile" role="tab" data-toggle="tab"><%=Resources.L.角色设置 %></a></li>
        <li class="nav-item"><a class="nav-link" href="#messages" role="tab" data-toggle="tab"><%=Resources.L.审核权限 %></a></li>
    </ul>
    <div class="tab-content container-fluid pr-0" >
        <div class="tab-pane active row sysRow" id="home">
            <table class="table table-striped table-bordered table-hover sys_table">
                <tr>
                    <th class="w12rem_lg"><%=Resources.L.管理员名称 %></th>
                    <td>
						...
						<small class="text-muted"><%=Resources.L.管理员名称_notice %></small>
                ...
                <tr>
                    ...
                    <td>
						...
						<small class="text-muted"><%=Resources.L.真实姓名_notice %></small>
                ...
                <tr>
                    ...
                    <td>
						...
						<small class="text-muted"><%=Resources.L.新密码_notice %></small>
                ...
                <tr>
                    ...
                    <td>
						...
						<small class="text-muted"><%=Resources.L.确认密码_notice %></small>
                 ...
                 <tr>
                    <th><%=Resources.L.二级密码 %></th>
                    <td>
                        <asp:TextBox ID="SPwd_T" runat="server" CssClass="form-control max20rem"></asp:TextBox>
						<small class="text-muted"><%=Resources.L.二级密码_notice %></small>
                     </td>
                </tr>
                <tr id="SetModel_Div" visible="false" runat="server">
                    <th><%=Resources.L.场景设置 %></th>
                    <td>
                        <asp:Label runat="server" ID="Theme_L"><span class="btn btn-outline-info btn-sm"><%=Resources.L.默认场景 %></span></asp:Label>
                        <input type="button" value="<%=Resources.L.配置 %>" class="btn btn-outline-info btn-sm" onclick="ShowAD();" />
                        <input type="button" value="<%=Resources.L.清除 %>" class="btn btn-outline-danger btn-sm" onclick="ClearAD();" />
                        <asp:HiddenField ID="curmodel_hid" runat="server" />
                    </td>
                </tr>
                <tr runat="server" visible="false">
                    <th><%=Resources.L.所属组织 %></th>
                    <td class="bqright">
                        <asp:TextBox ID="txtNode" CssClass="form-control max20rem"  runat="server" Enabled="false" Text="<%$Resources:L,组织结构 %>"></asp:TextBox>
                        <input id="Button3" type="button"  value="<%=Resources.L.更改组织 %>" onclick="AddNode('link')" class="btn btn-info"  />
                        <table cellpadding="0" cellspacing="0" id="NondeTable" border="0"></table>
                        <asp:HiddenField ID="hfNode" runat="server" />
                    </td>
                </tr>
                <tr>
                    <th><%=Resources.L.选项设置 %></th>
                    <td>
                        <div><asp:CheckBox ID="AddUser_Chk" Checked="true" runat="server" Text="<%$Resources:L,添加同名用户 %>" /></div>
                        <div><asp:CheckBox ID="cb1" Checked="true" runat="server" Text="<%$Resources:L,允许多人同时使用 %>" /></div>
                        <div><asp:CheckBox ID="cb2" Checked="true" runat="server" Text="<%$Resources:L,允许管理员修改密码 %>" /></div>
                        <div><asp:CheckBox ID="cb3"  runat="server" Text="<%$Resources:L,是否锁定 %>" /></div>
                        <div><asp:CheckBox ID="cb4" runat="server" Text="<%$Resources:L,是否启用个性桌面 %>" /></div>
                    </td></tr>
            </table>
        </div>
        <div class="tab-pane row sysRow" id="profile">
            <table class="table table-striped table-bordered table-hover sys_table">
                <tr>
                    <th class="w12rem_lg"><%=Resources.L.管理员角色设置 %></th>
                    <td>
        ...
        <div class="tab-pane row sysRow" id="messages">
            <table class="table table-striped table-bordered table-hover sys_table">
                <tr>
                    <th class="w12rem_lg"><%=Resources.L.是否前台审核互动 %></th>
                    <td><asp:CheckBox ID="CheckBox1" runat="server" /></td>
                </tr>
                <tr>
                    <th><%=Resources.L.发布内容默认状态 %></th>
                    <td>
                        <asp:DropDownList ID="DefaultStart_DP"  class="form-control w12rem_lg" runat="server">
                            <asp:ListItem Value="99" Selected="True" Text="<%$Resources:L,已审 %>"></asp:ListItem>
                            <asp:ListItem Value="0" Text="<%$Resources:L,待审 %>"></asp:ListItem>
                            <asp:ListItem Value="-1" Text="<%$Resources:L,退稿 %>"></asp:ListItem>
                        </asp:DropDownList>
                    </td>
                </tr>
            </table>
        ...
        <div class="sysBtline text-center">
            <asp:Button ID="btnSubmit" class="btn btn-info" runat="server" OnClick="btnSubmit_Click" Text="<%$Resources:L,保存信息 %>" />
            <a href="AdminManage.aspx" class="btn btn-outline-info"><%=Resources.L.返回列表 %></a>
        </div>
    </div>
	</div>
