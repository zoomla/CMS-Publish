...
<li class="breadcrumb-item"><%=Resources.L.方案列表 %> </li>
<div class="ml-auto"><a href="TemplateSetOfficial.aspx"><i class="zi zi_clouddownloadalt" zico="从云下载"></i>> <%=Resources.L.云模板下载 %> </a></div>
</ol>
			...
			<ItemTemplate>
				<li class="Template_list_card">
					<div class="Template_box">
						<div class="tempthumil"><a href="TemplateManage.aspx?setTemplate=<%#Eval("name") %>" title="<%=Resources.L.点击进入模板管理 %>">
						<span style="background: url(/Template/<%#Eval("name") %>/view.jpg) no-repeat top;"></span>						
						</div>
						<ul>
						<li class="temp_title">
						<a href="javascript:;" title="<%=Resources.L.作者 %> :<%# GetTlpName("Author")%>" data-toggle="tooltip" data-placement="top"><i class="zi zi_copyright" zico="版权"></i></a>
						<a href="TemplateManage.aspx?setTemplate=<%#Eval("name") %>" title='<%# GetTlpName("Project")%>'><%# GetTlpName("Project")%></a>
						</li>
							<li class="temp_isuse"><asp:LinkButton runat="server" ID="isUse" CommandArgument='<%#Eval("name") %>' CommandName="set"><%=Resources.L.状态 %> :</asp:LinkButton><%#IsDefaultTlp() %></li>
							<li class="temp_del">
							 <a href='/Template/<%#Eval("name") %>/view.jpg' class="lightbox"> <i class="zi zi_searchplus" zico="搜索放大"></i> <%=Resources.L.预览 %></a>
								<asp:LinkButton runat="server" CommandArgument='<%#Eval("name") %>' CommandName="del2" OnClientClick="return confirm('你确定删除吗');"> <i class="zi zi_trashalt" zico="垃圾箱竖条"></i> <%=Resources.L.删除 %> </asp:LinkButton>
							</li>
						</ul>
						</span>
