...
<script src="/Plugins/Third/bootstrap-select/js/bootstrap-select.js?v=1"></script>
<link href="/Plugins/Third/bootstrap-select/css/bootstrap-select.css?v=1" rel="stylesheet" />
...
.bootstrap-select:not([class*="col-"]):not([class*="form-control"]):not(.input-group-btn){width:20rem;border:1px solid #ddd;border-radius:4px;}
</style>
<title>标签查询</title>
</asp:Content>
          ...
          <td style="padding:0px;">
              <table class="table">
                  ...
                  <tr>
                        <th>主表</th>
                      <td>
                          <asp:DropDownList ID="TableList_DP" DataTextField="Name" DataValueField="Name" class="" runat="server" AutoPostBack="True" OnSelectedIndexChanged="TableList_DP_SelectedIndexChanged" /></td>
                      <th class="w12rem_lg">从表</th>
                      <td class="m50rem_50">
                          <asp:DropDownList ID="TableList2_DP" DataTextField="Name" DataValueField="Name" runat="server" AutoPostBack="True" OnSelectedIndexChanged="TableList2_DP_SelectedIndexChanged" /></td>
                  </tr>
                  <tr>
                      <th>输出字段</th>
                      <td>
      ...
      <tr>
		  <td>
              ...
              <div style="margin-left: 110px;">
                  ...
                  <asp:Button ID="SqlField_Btn" class="btn btn-outline-info btn-sm" runat="server" Text="查询字段" OnClick="SqlField_Btn_Click" style="margin-top:5px;" />
	  ...
	  <tr>
		...
		<td><table class="table table-striped table-bordered" style="text-align: center">
            ...
            <tr>
			    ...
			    <td>
                    <asp:Button ID="BtnAddParam" runat="server" Text="添加参数" class="input-group-text" OnClientClick="return VoteCheck();" OnClick="BtnAddParam_Click" Style="cursor: pointer;" />
	  ...
	  <tr>
		...
		<td>
            <div id="gridviewclause" class="input-group fielddiv m50rem_50">
                <asp:DropDownList ID="Where_Join_DP" CssClass="form-control" style="width:80px;" runat="server">
			      <asp:ListItem Selected="True">And</asp:ListItem>
			      <asp:ListItem>OR</asp:ListItem>
			    </asp:DropDownList>
			    <asp:DropDownList ID="Where_Table_DP" CssClass="form-control" style="width:100px;" runat="server" AutoPostBack="True" OnSelectedIndexChanged="Where_Table_DP_SelectedIndexChanged">
			    ...
			    <asp:DropDownList ID="Where_OPCode_DP" CssClass=" form-control" style="width:100px;" runat="server">
			    ...
			    <asp:TextBox ID="Where_Value_T" class="form-control" style="" runat="server"></asp:TextBox>
                <span class="input-group-append">
                    <asp:Button ID="Where_Btn" runat="server" class="btn btn-outline-secondary" Text="添加查询条件" OnClick="Where_Btn_Click" />
                </span>
            </div>
		...
		<td>
            <div id="Div2" class="fielddiv">
                <div class="input-group m50rem_50">
                    ...
                    <asp:TextBox runat="server" ID="OrderMode_T" class="form-control" style="border-right:none;width:80px;" />
                   
                    <span class="input-group-append">
					 <button type="button" class="btn btn-outline-secondary" onclick="$('#OrderMode_T').val('DESC')">降序</button>
                     <button type="button" class="btn btn-outline-secondary" onclick="$('#OrderMode_T').val('ASC')">升序</button>
                        <asp:Button ID="Order_Btn" class="btn btn-outline-secondary" runat="server" Text="添加排序字段" OnClick="Order_Btn_Click" ValidationGroup="false" />
                    </span>
                </div>
      ...
      <tr>
            <td colspan="2">
                <div>
					...
					<a href="LabelManage.aspx" class="btn btn-link" onclick="return confirm('放弃保存返回列表?');"><i class="zi zi_times" aria-hidden="true"></i> 取消</a>
                </div>
            </td>
        </tr>
	</table>
...
<script>
    ...
    function VoteCheck() {
        ...
        var name = $("#TxtParamName").val();
        if (ZL_Regex.isEmpty(name)) { alert("参数名不能为空"); return false; }
        return true;
    }
    ...
    var labelEditor = {
        ...
        save: function () {
            var ref = this;
            if ($("#" + ref.current).length < 1) { return true; }
