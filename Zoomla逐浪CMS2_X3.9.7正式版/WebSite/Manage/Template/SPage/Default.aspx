...
<asp:Content runat="server" ContentPlaceHolderID="head">
    <title><%=Resources.L.页面列表 %></title>
</asp:Content>
<asp:Content runat="server" ContentPlaceHolderID="Content">
    <%=Call.SetBread(new Bread[] {
		new Bread("/{manage}/Main.aspx",Resources.L.工作台),
        new Bread("Default.aspx",Resources.L.可视设计),						
        new Bread() {url="", text=Resources.L.页面列表 +"<a href='AddPage.aspx'> ["+Resources.L.添加页面 +"]</a>",addon="" }}
		)
    %>
                    ...
                    <div class="SPage_list_t">
                        <a href="/design/spage/Default.aspx?id=<%#Eval("ID") %>" target="_blank" title="<%#Eval("PageName")%>">
                            <div class="SPage_list_tc">
                                ...
                                <p><%=Resources.L.备注 %> ：<%#Eval("PageDESC") %></p>
                            </div>
                        </a>
                    </div>
                    <div class="SPage_list_b">
                        <a href="/design/spage/Default.aspx?id=<%#Eval("ID") %>" target="_blank"><i class="zi zi_paintbrush" zico="笔刷"></i> <%=Resources.L.设计 %></a>
                        <a href="<%#string.IsNullOrEmpty(Eval("ViewUrl",""))?"/design/spage/preview.aspx?id="+Eval("ID"):Eval("ViewUrl","") %>" target="_blank"><i class="zi zi_eye"></i> <%=Resources.L.预览 %></a>
                        <asp:LinkButton runat="server" CommandArgument='<%#Eval("ID") %>' CommandName="copy" OnClientClick="return confirm('确定要复制吗');"><i class="zi zi_copy"></i> <%=Resources.L.复制 %></asp:LinkButton>
                        <a href="AddPage.aspx?id=<%#Eval("ID") %>"><i class="zi zi_edit"></i> <%=Resources.L.修改 %></a>
                        <asp:LinkButton runat="server" CommandName="del2" CommandArgument='<%#Eval("ID") %>' OnClientClick="return confirm('确定要删除吗');"><i class="zi zi_trash"></i> <%=Resources.L.删除 %></asp:LinkButton>
                    </div>
                </div>
            </div>
