...
//------------------------------yBox-TaskAdd2.aspx
//v_int_all                           //允许正负整数
//v_int_zero                          //必须为正整数,允许0
//v_int                               //必须为正整数
//v_must                              //必填
//v_num                               //必须为数字(整数,浮点数,正负值均允许)
//v_num_region data-max="" data-min=""//字符必须为数字,必须在指定范围内
ZL_Regex.checkForm = function () {
    var $doms = $(".v_must");
    for (var i = 0; i < $doms.length; i++) {
        var $dom = $($doms[i]);
        if (ZL_Regex.isEmpty($dom.val())) { $dom.focus(); alert("数据不可为空"); return false; }
    }
    $doms = $(".v_int");
    for (var i = 0; i < $doms.length; i++) {
        var $dom = $($doms[i]);
        if (ZL_Regex.isEmpty($dom.val())) { continue; }
        var value = Convert.ToInt($dom.val(), -1);
        if (value < 0) { $dom.focus(); alert("数字必须>0"); return false; }
    }
    $doms = $(".v_num_region");
    for (var i = 0; i < $doms.length; i++) {
        var $dom = $($doms[i]);
        if (ZL_Regex.isEmpty($dom.val())) { continue; }
        var value = Convert.ToInt($dom.val());
        var max = Convert.ToInt($dom.data("max"));
        var min = Convert.ToInt($dom.data("min"));
        if (value > max || value < min) { $dom.focus(); alert("数据值必须在范围内"); return false; }
    }
}
//------------------------------
var Convert = {};
