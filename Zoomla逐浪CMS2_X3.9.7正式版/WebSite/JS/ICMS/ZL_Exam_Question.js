...
function SelQuestion(questList, id) {
    ...
    curScope.$apply(function ($compile) {
        for (var i = 0; i < questList.length; i++) {
            var content = $("<div>"+questList[i].p_title+"</div>").text();
            if (id > 0) {//修改
...
function ShowAdd(id , pid) {
    if ($("#p_title").val() == "") { alert("请填写试题标题!"); return; }
    comdiag.reload = true;
    ShowComDiag("/User/Exam/AddSmallQuest" + (id ? "?id=" + id : "") + (pid ? "?pid=" + pid : ""), "添加小题");
}
function ShowSel() {
    var url = "/User/Exam/SelQuestion?issmall=1&selids=," + $("#Qids_Hid").val() + ",";
...
var tabarr = [];//关键字数组
function CheckData() {
    var node = $("#TreeTlp_hid").val();
    if (ZL_Regex.isEmpty(node)||node==undefined||node==0) {
        alert("请选择试题科目");
        return false;
    }
    if (ZL_Regex.isEmpty($("#p_title").val())) { alert("标题不能为空"); return false; }
...
$(function () {
    ...
    //标题字数
    $("#p_title").bind("keydown paste cut blur", function () {
        setTimeout(function () { $("#titleNum_sp").html("字数：<strong style='color:green;'>" + $("#p_title").val().length) + "</span>"; }, 50);
    })
    //完型填空
