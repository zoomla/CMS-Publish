<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FastCreate.aspx.cs" Inherits="ZoomLaCMS.Design.mbh5.FastCreate" MasterPageFile="~/Design/Master/Empty.Master" %>
...
<div id="imgs_wrap">
    ...
    <a href="javascript:;" ng-repeat="item in imgs track by $index">
        <img ng-src="{{item.src}}" />
        <button type="button" class="button btn_del button-small button-assertive button-block" ng-click="delimg(item);"><i class="zi zi_trashalt" style="font-size:20px;"></i></button>
    </a>
</div>
<input type="file" id="file_up" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp" style="display:none;" onchange="picup.upload();" />
...
angular.module("app", []).controller("APPCtrl", function ($scope) {
    ...
    weui.toast.wait();
    $scope.imgs = [];
    //$scope.imgs.push({ id: Math.random(), src: "/UploadFiles/nopic.svg" });
    //$scope.imgs.push({ id: Math.random(), src: "/UploadFiles/nopic.svg" });
    //$scope.imgs.push({ id: Math.random(), src: "/UploadFiles/nopic.svg" });
    //$scope.imgs.push({ id: Math.random(), src: "/UploadFiles/nopic.svg" });
    //$scope.imgs.push({ id: Math.random(), src: "/UploadFiles/nopic.svg" });
