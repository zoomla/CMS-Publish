<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AppConfig.aspx.cs" Inherits="ZoomLaCMS.Manage.Config.AppConfig" MasterPageFile="~/Manage/I/Index.master" %>
...
<%=Call.SetBread(new Bread[] {
	new Bread("/{manage}/I/SiteInfo.aspx","系统设置"),
	new Bread("SiteInfo.aspx","网站配置"),
	new Bread() {url="", text="维护中心 [<a href='/tools/'>扩展维护</a>]",addon= "" }},
	Call.GetHelp(7)
	)
%>
...
<ul class="nav nav-tabs list_choice" role="tablist">
	<li role="presentation" class="nav-item"><a class="nav-link active show" href="#NormalConfig" aria-controls="NormalConfig" role="tab" data-toggle="tab">通用配置</a></li>
	<li role="presentation" class="nav-item"><a class="nav-link" href="#ErroCfg" aria-controls="ErroCfg" role="tab" data-toggle="tab">错误页配置</a></li>
  </ul>
<table class="table table-striped table-bordered table-hover tab-content col-12 sys_table">
	<tbody role="tabpanel" id="NormalConfig" class="tab-pane active">
	<tr>
		<td class="tdleft"><strong><%:lang.LF("是否重新安装") %></strong></td>
		<td>
			<input type="checkbox" runat="server" id="Installed_Chk" class="switchChk" /><span class="text-success"> 点击启动重系统请慎重!</span>
		</td>
	</tr>
	<tr>
			...
			<ul ng-controller="appController" class="default_div">
				...
				<li ng-repeat="item in list">
					<div class="input-group margin_b2px ">
						...
						<span class="input-group-btn">
							<button type="button" class="btn btn-default" ng-click="remove($index)"><span class="zi zi_trashalt"></span></button>
							<button type="button" class="btn btn-default" ng-click="moveup($index)"><span class="zi zi_arrowUp"></span></button>
						</span>
					</div>
				</li>
			</ul>
	...
	<tr>
		<th class="text-right"><strong>启用错误页</strong></th>
		<td>
					...
					<ItemTemplate>
						...
						<div class="input-group">
						  <div class="input-group-prepend">
							<span class="input-group-text"><%#Eval("ErrCode") %></span>
						  </div>
						  <asp:HiddenField runat="server" ID="ErrCode_Hid" Value='<%#Eval("ErrCode") %>' />
						  <asp:TextBox runat="server" ID="ErrUrl_T" class="form-control required serverurl" Text='<%#Eval("Url") %>' />
						</div>
						</li>
					</ItemTemplate>
				</asp:Repeater>
...
<asp:Button ID="Button1" runat="server" Text="保存设置" OnClientClick="return UrlChk();" OnClick="Button1_Click" class="btn btn-outline-info" />
</asp:Content>
