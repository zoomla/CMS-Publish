    ...
    <ZL:ExGridView ID="EGV" runat="server" AutoGenerateColumns="False" PageSize="10" OnPageIndexChanging="EGV_PageIndexChanging" IsHoldState="false" OnRowCommand="EGV_RowCommand" OnRowDataBound="EGV_RowDataBound"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="操作">
                <ItemTemplate>
                    <a href="AddExpSender.aspx?id=<%#Eval("ID") %>"><i class="zi zi_pencilalt"></i>修改</a>
                    <span class="<%#Eval("IsDefault","")=="1" ? "enabled" : "" %>">
                        <asp:LinkButton runat="server" CommandName="setdef" Enabled='<%#Eval("IsDefault","")!="1" %>' CommandArgument='<%#Eval("ID") %>'><i class="zi zi_check"></i>设为默认</asp:LinkButton></span>
                    <asp:LinkButton runat="server" CommandName="del" CommandArgument='<%#Eval("ID") %>' OnClientClick="return confirm('你确定将该数据删除吗？');"><i class="zi zi_trashalt"></i>删除</asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>
