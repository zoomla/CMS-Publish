    ...
    <div class="container-fluid margin_t5">
        <div class="list-group">
          <ZL:Repeater ID="RPT" runat="server">
              <ItemTemplate>
                  <a href="javascript:;"  onclick="parent.GetModelID('<%=Request.QueryString["cid"] %>','<%#Eval("ModelID") %>')" class="list-group-item"><span class="<%#Eval("ItemIcon") %>"></span><strong><%# Eval("ModelName")%></strong> [<%#Eval("ModelID") %>]</a>
              </ItemTemplate>
          </ZL:Repeater>
            <ZL:Repeater ID="Fied_RPT" runat="server">
              <ItemTemplate>
                  <a href="javascript:;"  class="list-group-item selfield"><%#Eval("FieldName")%>(<%#Eval("FieldAlias")%>)<input name="fields" style="display:none;" value="<%#Eval("FieldName")%>" type="checkbox" /><span class="badge" style="display:none;"><span class="zi zi_check"></span></span></a>
              </ItemTemplate>
          </ZL:Repeater>
        </div>
