<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SiteDataCenter.aspx.cs" Inherits="ZoomLaCMS.Manage.Site.SiteDataCenter" MasterPageFile="~/Manage/I/Index.master" ValidateRequest="false"%>
...
<table class="table table-striped table-bordered table-hover">
    <tr>
        <td class="w12rem_lg"><input type="checkbox" id="siteChkAll" title="选择全部" onclick="selectAll(this, 'siteChk')"/>站点名</td><td >网址</td>
    </tr>
  <asp:Repeater runat="server" ID="siteRepeater">
      <ItemTemplate>
      <tr>
         <td><input type="checkbox" name="siteChk" value='<%# GetDomain(Eval("Domain").ToString(),Eval("SitePort").ToString()) %>' />
...
<div class="input-group">
    ...
    <asp:Button runat="server" ID="Button1" Text="开始采集" OnClick="beginGet_Click" OnClientClick="disSiteInfo();" CssClass="btn btn-outline-info"  />
    </span>
</div> 
...
<div class="rightDiv-Head" >
       <%-- <asp:TextBox runat="server" ID="siteUrl" Width="500px" TextMode="MultiLine"></asp:TextBox>
        ...
        <span  id="siteInfoSpan">采集站点：</span>
		<div>
		<div class="input-group mb-3">
		  <div class="input-group-prepend">
			<span class="input-group-text">批量修改</span>
		  </div>
		  <asp:DropDownList runat="server" ID="mainDp1" CssClass="form-control max20rem"></asp:DropDownList>
		  <div class="input-group-append">
			<span class="input-group-text">为</span>
		  </div>
		  <asp:DropDownList runat="server" ID="mainDp2"  CssClass="form-control max20rem"></asp:DropDownList>
		</div>
		</div>
        <input type="button" value="确定" onclick="changeNode()" class="btn btn-sm btn-outline-info"/>
        <asp:Button runat="server" ID="btn1" Text="采集入库"  OnClick="btn1_Click" CssClass="btn btn-sm btn-outline-info" OnClientClick="return checkData()"/>
        <asp:Button runat="server" ID="logBtn" Text="查看日志" OnClick="logBtn_Click" CssClass="btn btn-sm btn-outline-info" />
