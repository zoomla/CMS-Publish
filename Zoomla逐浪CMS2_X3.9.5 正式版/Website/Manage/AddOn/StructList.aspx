            ...
            <ZL:ExGridView runat="server" ID="EGV" AutoGenerateColumns="false" AllowPaging="true" PageSize="10" EnableTheming="False" IsHoldState="false"
                ...
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="操作">
                        <ItemTemplate>
                             <asp:LinkButton CommandName="Edit" CommandArgument='<%#Eval("ID") %>' ToolTip="修改" runat="server"><i class="zi zi_squareCorrect" title="修改"></i></asp:LinkButton>
                            <a href='javascript:;' onclick='delstruct(<%#Eval("ID") %>)'><i class='zi zi_trashalt' title="删除"></i></a>
                            <a href="StructMenber.aspx?id=<%#Eval("ID") %>" title="成员管理"><i class="zi zi_useralt" title="管理"></i>成员管理</a>
                            <asp:LinkButton ID="Add_lbn" CommandName="Add" CommandArgument='<%#Eval("ID") %>' Text="添加子级" runat="server"><i class="zi zi_plus"></i>添加子级</asp:LinkButton>
                            <a <%#IsHasChild(Eval("ID")) %> ><i class="zi zi_eye"></i>查看子级</a>
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
            </ZL:ExGridView>
...
<asp:Content runat="server" ContentPlaceHolderID="ScriptContent">
    ...
    <script type="text/javascript">
        ...
        var trtlp = "<tr data-pid='@pid' data-index='@index'><td><input name='idchk' type='checkbox' value='@id'></td><td>@id</td><td>@treeindex<img src='/Images/TreeLineImages/t.gif' border='0' /><a href='AddStruct.aspx?ID=@id'>@icon @title</a></td>"
                        ...
                        + "<td><a href='StructMenber.aspx?id=@id' title='成员管理'><i class='zi zi_useralt'></i></a> <a href='AddStruct.aspx?ID=@id&type="+type+"'><i class='zi zi_squareCorrect'></i></a> "
                        + "<a href='javascript:;' onclick='delstruct(@id)'><i class='zi zi_trashalt'></i></a> <a href='AddStruct.aspx?pid=@id&type="+type+"'><i class='zi zi_plus'></i>添加子级</a></td></tr>";
        $().ready(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < datas.length; i++) {
                        ...
                        .replace(/@icon/g, datas[i].childCount > 0 ? "<span class='icon zi zi_pluscircle'></span>" : "");
                    }
                    $(html).insertAfter($tr);
                    $tr.unbind('click');
        ...
        function TrDisplayEvent($tr,$parent) {
            var id = $tr.find("[name='idchk']").val();
            if ($tr.is(":hidden")) {
                $parent.find('.icon').removeClass('zi zi_minussquare').addClass('zi zi_plusSquare');
                $tr.parent().find("[data-pid='" + id + "']").hide();
                $tr.parent().find("[data-pid='" + id + "']").trigger('ondisplay');
            } else {
                $parent.find('.icon').removeClass('zi zi_plusSquare').addClass('zi zi_minussquare');
            }
        }
