...
<div class="treediv">
    <ul>
        <li><span class='zi zi_listalt' style="color:#999;"></span><%=liAllTlp %></li>
...
<script>
    $().ready(function () {
        ...
        if (isshrink == "True") {
            ...
            $('.treeicon').removeClass("zi zi_minuscircle");
            $('.treeicon').addClass("zi zi_pluscircle");
        }
        var nodeid = "<%=SelectedNode %>";//初始节点id(选中节点)
        SelectNode($('.treediv [data-id=' + nodeid + ']'));
        InitTreeEvent();
    });
    var activetlp = "<span class='zi zi_arrowLeft tree_active'></span>";//选中模板
    //初始化树形事件
    ...
    function ExtendNode($gen) {
        ...
        $gen.find(".treeicon").first().removeClass("zi zi_pluscircle");
        $gen.find(".treeicon").first().addClass("zi zi_minuscircle");
    }
    //收缩节点，参数为根节点(包含ul标签的li)
    function ShrinkNode($gen) {
        $gen.removeClass("showchild");
        $gen.find("ul:first").hide();
        $gen.find(".treeicon").first().removeClass("zi zi_minuscircle");
        $gen.find(".treeicon").first().addClass("zi zi_pluscircle");
    }
</script>