...
<div class="tab-content col-12 table_box">
  <ZL:ExGridView ID="EGV" runat="server" AllowPaging="True" DataKeyNames="ID" OnRowDataBound="EGV_RowDataBound" 
	  ...
	  <Columns>
          ...
          <asp:TemplateField HeaderText="等级" ItemStyle-CssClass="">
              <ItemTemplate>
                  <input type="text" class="form-control orderItem w12rem_lg" data-id="<%#Eval("ID") %>" value="<%#Eval("OrderID") %>"/>
              </ItemTemplate>
          </asp:TemplateField>
		  <asp:BoundField DataField="Remark" HeaderText="备注" ItemStyle-CssClass=""/>
		  <asp:TemplateField HeaderText="<%$Resources:L, 操作%>" ItemStyle-CssClass="">
              <ItemTemplate>
                  <a href="AddPointGroup.aspx?ID=<%#Eval("ID") %>"><i class="zi zi_pencilalt" title="<%=Resources.L.修改 %>"></i></a>
                  <asp:LinkButton runat="server" CommandArgument='<%#Eval("ID") %>' CommandName="del2"
                      OnClientClick="return confirm('确认删除？')"><i class="zi zi_trashalt"></i>删除</asp:LinkButton>
              </ItemTemplate>
          </asp:TemplateField>
	  </Columns>
  </ZL:ExGridView>
