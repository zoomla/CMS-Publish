...
<%=Call.SetBread(new Bread[] {
	...
	new Bread() {url="", text="会员"+GetTypeName()+"操作",addon="" }}
	)
%>
...
<div id="UserPDiv" runat="server">
	<table class="table table-striped table-bordered table-hover sys_table">
		...
		<tr>
			<th><%=GetTypeName() %></th>
			<td>
				<asp:TextBox ID="TxtScore" runat="server" class="form-control max20rem num" autocomplete="off" ValidationGroup="submit"></asp:TextBox>
				<asp:RegularExpressionValidator runat="server" Display="Dynamic" ID="RV2" ValidationExpression="^(\d*\.\d{0,2}|[1-9]+).*$" ControlToValidate="TxtScore" ErrorMessage="只能输入非0整数" ForeColor="Red" ValidationGroup="submit"/>
				<asp:RequiredFieldValidator runat="server" Display="Dynamic" ID="ScoreVa" ControlToValidate="TxtScore" ErrorMessage="不能为空!" ForeColor="Red" ValidationGroup="submit"/>
			</td>
		</tr>
		<tr>
			<th>上传凭证</th>
		...
		<tr>
			<td></td>
			<td>
				<asp:Button ID="EBtnSubmit" Text="确定" OnClick="EBtnSubmit_Click" runat="server" class="btn btn-outline-info" OnClientClick="disBtn(this,2000);" ValidationGroup="submit"/>
	...
	<ZL:ExGridView ID="EGV" runat="server" AutoGenerateColumns="False" DataKeyNames="ExpHisID" PageSize="20" OnPageIndexChanging="EGV_PageIndexChanging" IsHoldState="false" OnRowCommand="EGV_RowCommand" AllowPaging="True" AllowSorting="True" CellPadding="2" CellSpacing="1" BackColor="White" ForeColor="Black" CssClass="table table-striped table-bordered table-hover" EnableTheming="False" GridLines="None" EnableModelValidation="True">
		<Columns>
			<asp:BoundField HeaderText="流水号(ID)" DataField="ExpHisID" HeaderStyle-Height="22" />
...
<table id="EditMoney" class="table table-bordered table-hover sys_table" runat="server" visible="false">
	<tr>
		<th scope="row" class="w12rem_lg">流水号(ID)</th>
		<td>
	...
	<tr>
		...
		<td>
			<asp:TextBox ID="HisTime" CssClass="form-control max20rem" runat="server" onclick="WdatePicker({ dateFmt: 'yyyy/MM/dd HH:mm:ss' });"></asp:TextBox>
		</td>
	</tr>
	<tr>
		<th scope="row">会员<%=GetTypeName() %></th>
		<td>
			<asp:TextBox ID="Score" CssClass="form-control max20rem" runat="server" Text="0"></asp:TextBox>
		 ...
		 <small class="text-muted">此处定义只修改流水记录不影响用户最终余额-非平帐操作。</small>
		</td>
	</tr>
	<tr>
		<th scope="row">详细</th>
	...
	<tr>
		<td></td>
		<td>
			<asp:Button ID="EditBtn" Text="修改" class="btn btn-outline-info" OnClick="EditBtn_Click" runat="server" />
			<a href="UserExp.aspx?UserID=<%=UserID %>&type=<%=ExpType %>" class="btn btn-outline-info">取消</a>
		</td>
	</tr>
</table>
