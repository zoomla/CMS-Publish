    ...
    <ZL:ExGridView ID="EGV" runat="server" AutoGenerateColumns="false" PageSize="10" IsHoldState="false"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="操作">
                <ItemTemplate>
                    <asp:LinkButton runat="server" CommandArgument='<%#Eval("name") %>' CommandName="download"  ToolTip="下载"><i class="zi zi_clouddownload"> </i></asp:LinkButton>
                    <asp:LinkButton runat="server" CommandArgument='<%#Eval("name") %>' CommandName="restore"  OnClientClick="return confirm('[确定恢复该备份吗?]改为[系统将还原所有节点模型数据(此操作不可逆),是否继续?] 恢复前清空数据');"><i class="zi zi_recycle"> 数据恢复</i></asp:LinkButton>
                    <asp:LinkButton runat="server" CommandArgument='<%#Eval("name") %>' CommandName="del2"  OnClientClick="return confirm('确定要删除该备份吗?');"><i class="zi zi_trash"> 删除备份</i></asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>
