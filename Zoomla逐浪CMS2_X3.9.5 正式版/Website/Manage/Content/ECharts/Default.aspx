    ...
    <ZL:ExGridView ID="EGV" runat="server" AutoGenerateColumns="False" PageSize="10" OnRowCommand="EGV_RowCommand"
        ...
        <Columns> 
            ...
            <asp:TemplateField HeaderText="操作">
                <ItemTemplate>
                   <a href="/Plugins/ECharts/ShowM.aspx?ID=<%#Eval("ID") %>" target="_blank" title="预览"><span class="zi zi_eye" title="预览"></span></a>
                    <a href="AddChart2.aspx?<%#"id="+Eval("ID")+"&type="+Eval("SType")+"&tag="+Eval("Tag") %>" ><span class="zi zi_pencilalt" title="编辑"></span> </a>
                    <asp:LinkButton runat="server" CommandArgument='<%#Eval("ID") %>'  CommandName="del2" OnClientClick="return confirm('确定要删除吗?');"><span class="zi zi_trash" title="删除"></span> </asp:LinkButton>
                    <a href="ChartCite.aspx?ID=<%#Eval("ID") %>"  title="引用"><span class="zi zi_sharealt"></span> 引用</a>
                </ItemTemplate>
            </asp:TemplateField>         
        </Columns>
    </ZL:ExGridView>
