<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SurveyResult.aspx.cs" Inherits="ZoomLaCMS.Manage.Plus.SurveyResult" MasterPageFile="~/Manage/I/Index.master" %>
<asp:Content runat="server" ContentPlaceHolderID="head"><title>结果分析</title></asp:Content>
<asp:Content runat="server" ContentPlaceHolderID="Content">
    <%=Call.SetBread(new Bread[] {
        new Bread("/{manage}/I/Main.aspx","工作台"),
		new Bread("SurveyManage.aspx","问卷投票"),
        new Bread() {url="", text="结果分析[我的场景]",addon="" }}
		)
    %>
        ...
        <ItemTemplate>
            <div class="item <%#Eval("QType") %>" id="item_<%#Eval("ID") %>">
                ...
                <div class="chart_wrap excel_no text-center" <%#ShowChartWrap() %>>
                    <button type="button" class="btn btn-xs btn-info" onclick="showchart('<%#Eval("ID") %>','pie')"><i class="zi zi_chartpie"></i>饼状</button>
                    <button type="button" class="btn btn-xs btn-info" onclick="showchart('<%#Eval("ID") %>','circle')"><i class="zi zi_lifering"></i>圆环</button>
                    <button type="button" class="btn btn-xs btn-info" onclick="showchart('<%#Eval("ID") %>','bar')"><i class="zi zi_chartbar"></i>柱状</button>
                    <button type="button" class="btn btn-xs btn-info" onclick="showchart('<%#Eval("ID") %>','line')"><i class="zi zi_bars"></i>条形</button>
                    <button type="button" class="btn btn-xs btn-danger" onclick="hidechart('<%#Eval("ID") %>')"><i class="zi zi_expandarrowsalt"></i>关闭</button>
...
<div class="Conent_fix">
    <a href="/Vote/ResultAdd?ID=<%:Mid%>" class="btn btn-info" target="_blank"><i class="zi zi_plus"></i> 添加投票</a>
