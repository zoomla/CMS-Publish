...
<asp:Content  ContentPlaceHolderID="Content" Runat="Server">
    ...
    <asp:Button ID="delBtn" runat="server" Text="删除" OnClick="DelBtn_Click" UseSubmitBehavior="False"
        OnClientClick="if(checkedNum()){if(!confirm('确定要批量删除吗？')){return false;}}else{return false;}" class=" btn btn-info" />
    <asp:Button ID="Button2" runat="server" Text="审核通过" OnClick="BtnSubmit2_Click"  UseSubmitBehavior="False" class="btn btn-info" />
    <asp:Button ID="Button3" runat="server" Text="取消审核" OnClick="BtnSubmit3_Click"  UseSubmitBehavior="False" class="btn btn-info" />
    <input type="button"  value="回复问题" class="btn btn-info" onclick="dialog('管理员回复');" />
    <br />
    
    <div id="replyDiv" style="display:none;">
        <div class="container-fluid text-center">
            <textarea id="txtContent" style="height: 300px; width: 100%; margin-top: 10px;" runat="server"></textarea>
            <input type="button" id="addBtn" value="管理员回复" onclick="if (addReply()) { disableBtn(this); }" class="btn btn-info" />
            <input type="button" id="updateBtn" value="修改回复" onclick="updateReply()" class="btn btn-info" style="display: none;" />
            <input type="button" value="返回提问列表" class="btn btn-info" onclick="location = 'WdCheck.aspx'" style="margin-left: 5px;" />
        </div>
    </div>
