...
namespace ZoomLaCMS.Tools
{
    public partial class Default : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (adminMod.AdminId != 1) { function.WriteErrMsg("该页面仅允许超级管理员访问"); }
            else
            {
                SupperGavel.Visible = false;
                admintop.Visible = true;
            }
            if (!IsPostBack)
            {
        ...
        protected void Update_Btn_Click(object sender, EventArgs e)
        {
            ...
            SiteConfig.SiteInfo.LogoUrl = "/images/logo_w.svg";
            ...
            SiteConfig.SiteOption.AdvertisementDir = "/AD/";
            SiteConfig.SiteOption.UAgent = true;
            SiteConfig.SiteOption.DesignIsClose = true;
            SiteConfig.SiteOption.VerifyType = 0;
            SiteConfig.SiteOption.Admin_StartUrl = "/Manage/ProFile/WorkTable.aspx";
            //PWA
            SiteConfig.SiteOption.PWA_Name = "逐浪软件";
            SiteConfig.SiteOption.PWA_ShortName = "逐浪软件";
            SiteConfig.SiteOption.PWA_StartUrl = "/default.aspx";
            SiteConfig.SiteOption.PWA_Icon = "";
            //短信配置
            SiteConfig.SiteOption.DefaultSMS = "0";
            //商城配置
            SiteConfig.ShopConfig.OrderExpired = 72;
            //用户配置
            SiteConfig.UserConfig.MaximumUser = 0;
            SiteConfig.UserConfig.MobileReg = false;
            SiteConfig.UserConfig.EnableCheckCodeOfLogin = "2";
            SiteConfig.UserConfig.UserNavBan = "";
            ...
            SiteConfig.UserConfig.PresentMoney = 0;//注册赠送积分与余额
            SiteConfig.UserConfig.PresentExp = 0;
            SiteConfig.UserConfig.PresentExpPerLogin = 0;
            SiteConfig.UserConfig.PresentPoint = 0;

            //虚拟币
            SiteConfig.UserConfig.PointMoney = 0;
            SiteConfig.UserConfig.PointSilverCoin = 0;
            //水印
            WaterModuleConfig.WaterConfig.EnableUserWater = false;
            WaterModuleConfig.WaterConfig.IsUsed = true;
            WaterModuleConfig.Update();
            //--------------------
            ...
            rupdate.Update(HttpContext.Current, "admin");
            //--------------------
            SiteConfig.Update();
            //-----其他修改
            {
        ...
        protected void Check_Btn_Click(object sender, EventArgs e)
        {
            ...
            foreach (string filename in files)
            {
                ...
                {
                    existIcon = "zi zi_expandarrowsalt";
                }
                else
                {
                    existIcon = "zi zi_check";
                }
        ...
        protected void CloseDomainMerge_Btn_Click(object sender, EventArgs e)
        {
            SiteConfig.SiteOption.DomainMerge = false;
            SiteConfig.Update();
        }
        ...
        {
            string tbname = Import_Tbname_T.Text.Trim();
            if (string.IsNullOrEmpty(tbname)) { function.WriteErrMsg("表名不能为空"); }
            if (!DBHelper.Table_IsExist(tbname)) { function.WriteErrMsg("指定的表[" + tbname + "]不存在"); }
            DataTable dt = DBCenter.Sel(tbname, "1=2");
            dt.TableName = tbname;
            //使用xml后缀会被浏览器提示不安全
            string xmlPath = ZLHelper.GetUploadDir_System("import") + tbname + ".txt";
            Import_Logical.CreateXMLByDT(dt, Server.MapPath(xmlPath));
            SafeSC.DownFile(xmlPath, tbname + ".txt");
        }
    }
}