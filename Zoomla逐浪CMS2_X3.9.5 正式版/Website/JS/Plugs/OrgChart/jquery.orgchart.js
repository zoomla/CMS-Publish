...
(function(factory) {
  ...
  $.fn.orgchart = function(options) {
    ...
    if ($.type(data) === 'object') {
      ...
      $.ajax({
        ...
        'dataType': 'json',
        'beforeSend': function () {
          $chart.append('<i class="zi zi_redoalt zi_pulse spinner"></i>');
        }
      })
    ...
    if (opts.exportButton && !$chartContainer.find('.oc-export-btn').length) {
      var $exportBtn = $('<button>', {
        ...
        'click': function() {
          ...
          if (!$mask.length) {
            $chartContainer.append('<div class="mask"><i class="zi zi_redoalt zi_pulse spinner"></i></div>');
          } else {
  ...
  function startLoading($arrow, $node, options) {
    ...
    $node.append('<i class="zi zi_redoalt zi_pulse spinner"></i>');
  ...
  function createNode(nodeData, level, opts) {
    ...
    if(Number(flags.substr(2,1))) {
      $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa"></i>')
        .children('.title').prepend('<i class="zi '+ opts.parentNodeSymbol + ' symbol"></i>');
    }
    ...
    buildChildNode.call($node.closest('.orgchart').parent(), $node.closest('table'), data, opts, function() {
      if (++count === data.children.length) {
        ...
        if (!$node.find('.symbol').length) {
          $node.children('.title').prepend('<i class="zi '+ opts.parentNodeSymbol + ' symbol"></i>');
        }
        showDescendants($node);
      }
    });
