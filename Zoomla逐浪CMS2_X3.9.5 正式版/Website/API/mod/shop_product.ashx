                ...
                case "list":
                    {
                        ...
                        DataTable dt = proBll.SelPage(cpage, psize, -100, 0, "", 1).dt;
                ...
                case "search":
                    {
                        ...
                        DataTable dt = proBll.SelPage(cpage, psize, -100, 0, proname, 1).dt;
                        retMod.result = JsonConvert.SerializeObject(dt);
                        retMod.retcode = M_APIResult.Success;
                    }
                    break;
                case "region_price":
                    {
                        ...
                        int proid = DataConvert.CLng(Req("id"));
                        ZoomLa.BLL.Shop.B_Shop_RegionPrice regBll = new ZoomLa.BLL.Shop.B_Shop_RegionPrice();
                        //获取自己所匹配的区域价格
                        double price = regBll.GetRegionPrice(proid, 0, basemu.Province + "|" + basemu.City + "|" + basemu.County, mu.GroupID);
                        if (price > 0)
                        {
                            retMod.retcode = M_APIResult.Success;
                            retMod.result = price.ToString("F2");
                        }
                        else { retMod.retmsg = "未匹配地区"; }
                    }
                    break;
                default:
                    {
