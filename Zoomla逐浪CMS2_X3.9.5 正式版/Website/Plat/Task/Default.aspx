...
<div class="platcontainer container">
    ...
    <div class="input-group nav_searchDiv mt-2">
        <asp:TextBox runat="server" ID="searchText" class="form-control max20rem" placeholder="请输入需要搜索的内容" />
        ...
        <span class="input-group-append">
            <asp:LinkButton runat="server" CssClass="btn btn-info" ID="searchBtn" OnClick="searchBtn_Click"><i class="zi zi_search"></i> 搜索</asp:LinkButton>
            <a href="AddTask.aspx" class="btn btn-info" ><i class="zi zi_plus"></i> 创建新任务</a>
        </span>
    </div>
<div class="mt-2">
        <ZL:ExGridView runat="server" ID="EGV" AutoGenerateColumns="false" AllowPaging="true" PageSize="10" EnableTheming="False" IsHoldState="false"
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="操作">
                    <ItemTemplate>
                        <asp:LinkButton ID="Edit_Link" runat="server" CommandName="Edit" CommandArgument='<%#Eval("ID") %>' ToolTip="编辑"><i class="zi zi_pencilalt"></i>修改</asp:LinkButton>
                        <asp:LinkButton ID="LinkButton" runat="server" CommandName="Del" CommandArgument='<%#Eval("ID") %>' OnClientClick="return confirm('你确定要删除吗!');" ToolTip="删除"><i class="zi zi_trashalt"></i>删除</asp:LinkButton>
                    </ItemTemplate>
                </asp:TemplateField>
            </Columns>
        </ZL:ExGridView>
        <asp:Button runat="server" ID="DelBtn" OnClientClick="return confirm('你确定要删除吗!');" OnClick="DelBtn_Click" CssClass="btn btn-outline-danger" Text="批量删除" />
    </div>
</div>
</asp:Content>
