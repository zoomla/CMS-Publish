	...
	<ItemTemplate>
	  <div class="msgitem" id="msgitem-<%#Eval("ID") %>">
		...
		<div class="msg_content_div plat_content_div">
          ...
          <span class="grayremind" title="哪些人可见"><%#GetWhoCanSee()%></span><span class="grayremind" title="阅读量"> <i class="zi zi_eye"></i><%#Eval("ReadNumber") %></span>
		  <div class="reply_op_div"> <%#GetReplyOP() %> <span class="zi zi_commentdots" title="回复" onclick="DisReplyOP(<%#Eval("ID") %>,0,'<%#GetUName() %>');"></span> </div>
		  <div class="likeids_div" >
			<ul class="likeids_div_ul" style='<%#IsShowLike()%>'>
			  <li style="width:auto;"> <a href="javascript:;" onclick="PostLike(<%#Eval("ID") %>)" class="likeids_div_a"><span class="zi zi_thumbsup"></span><span class='likenum_span'>(<%#GetLikeNum() %>)</span></a> </li>
			      <%#ShowLikeUser() %>
			</ul>
		  </div>
		  ...
		  <div id="reply_div_<%#Eval("ID") %>" class="reply_item">
			...
			<div class="margin_t5">
                <a href="javascript:;" title="附件" class="reply_a_btn" onclick="reply.upfile('<%#Eval("ID") %>');"><i class="zi zi_paperclip zi_2x"></i></a>
