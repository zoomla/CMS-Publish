...
var FrameTab = {
    cfg: {
        startUrl: "Main",
        tabTlp: '<li class="FrameTab" id="FrameTab{tabId}" data-id="{tabId}" onclick="FrameTab.switchTab(this);"><a href="javascript:"><span class="FrameTabTitle">New Tab</span></a><a class="closeTab" title="close" onclick="FrameTab.del(this);"><span class="zi zi_times"></span></a></li>',
        iframeTlp: ''
    },
    cache: {
    ...
    init: function (json) {
        var ref = this;
        if (!json) {json = {}; }
        if (json.startUrl) { ref.cfg.startUrl = json.startUrl; }
        ref.cfg.iframeTlp = '<iframe id="main_right" name="main_right" class="FrameIframe" data-id="{tabId}" onload="FrameTab.setTitle(this)" src="' + ref.cfg.startUrl + '" style="z-index: 2; visibility: inherit; overflow-x: hidden; width: 100%; display: block;" frameborder="0" scrolling="yes"></iframe>';
    },
    //单击创建新tab
    add: function (addDom) {
        var ref = this;
