...
function Tlp_initTemp() {
    ...
    $(".Template_files").each(function (i, v) {
        ...
        if ($hid.val() != "")
        {
            $parent.find('button').html($hid.val() + " <span class='float-right'><span class='caret'></span></span>");
        }
    });
}
...
function Tlp_SetVal(obj, val) {
    ...
    $parent.find('button').html(val + " <span class='float-right'><span class='caret'></span></span>");
...
function cmsAlert(text, cb) {
    ...
    if ($modal.length > 0) {
        ...
        var html = "<div class=\"modal fade animated  sysart\" id=\"modal_alert\" tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"modal_alert_title\" aria-hidden=\"true\" data-backdrop='static'>"
            ...
            + "<div class=\"modal-header\"><h5 class=\"modal-title\" id=\"modal_alert_title\"><i class=\"zi zi_bulbGlow\"></i> <span class=\"modal-title-text\">" + data.title + "</span></h5><button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-label=\"Close\"><span aria-hidden=\"true\">&times;</span></button></div>"
            + "<div class=\"modal-body\">" + data.content + "</div>"
            + "<div class=\"modal-footer\">"
            + "<button type=\"button\" id=\"modal_alert_btn\" class=\"btn btn-outline-info btn-sm \" data-dismiss=\"modal\"><i class=\"zi zi_check\"></i> 确认</button>"
            + "</div></div></div></div>";
        $("body").append(html);
