...
(function ($) {
  ...
  Selectpicker.prototype = {
    ...
    createDropdown: function () {
      ...
      drop =
        '<div class="dropdown bootstrap-select' + showTick + '">' +
          '<button type="button" class="' + this.options.styleBase + ' dropdown-toggle" ' + (this.options.display === 'static' ? 'data-display="static"' : '') + autofocus + ' role="button">' +
          ...
          '</button>' +
      '<div class="' + classNames.MENU + ' ' + (version.major === '4' || version.major === '5' ? '' : classNames.SHOW) + '">' +
            header +
            searchbox +
            actionsbox +
            '<div class="inner ' + classNames.SHOW + '" role="listbox" aria-expanded="false" tabindex="-1">' +
                '<ul class="' + classNames.MENU + ' inner ' + (version.major === '4' || version.major === '5' ? classNames.SHOW : '') + '">' +
                '</ul>' +
            '</div>' +
            donebutton +
          '</div>' +
    ...
    createLi: function () {
      ...
      var generateA = function (text, classes, inline) {
        ...
        if (version.major === '4' || version.major === '5') a.classList.add('dropdown-item');
    ...
    liHeight: function (refresh) {
      ...
      menuInnerInner.className = classNames.MENU + ' inner ' + (version.major === '4'|| version.major === '5' ? classNames.SHOW : '');
    ...
    setMenuSize: function (isAuto) {
      ...
      this.$menu.css({
        // 20200821 LingJing 下拉超出边界或定位出现问题 加上-160
        'max-height': (maxHeight - 160) + 'px',
        'overflow': 'hidden',
    ...
    setDisabled: function (index, disabled, liIndex, li) {
      ...
      if (a) {
        if (version.major === '4'|| version.major === '5') a.classList.toggle(classNames.DISABLED, disabled);
    ...
    clickListener: function () {
      var that = this,
          $document = $(document);
        // 20200820 LingJing 添加事件监听，打开和关闭下拉框
        this.$button.click(function () {
            if (that.$menu.hasClass("show")) {
                that.$menu.removeClass("show");
            }
            else {
                that.$menu.addClass("show");
                that.$searchbox.val('');
            }
        });
      ...
      this.$newElement.on('show.bs.dropdown', function () {
        if (version.major > 3 && !that.dropdown) {
          ...
          // 20200821 lingjing dropdown._menu设置后貌似没使用过，而且会产生报错
      ...
      // 20200828 lingjing 新增搜索框回车选择第一个
      this.$searchbox.on('keypress', function (e) {
        if (e.which == 13) {
            e.preventDefault();
            that.$menuInner.find("li:first-child a:first-child").click();
        }
      });
    ...
    liveSearchListener: function () {
      var that = this,
        ...
        if (!!that.$searchbox.val()) {
          // 20200820 lingjing 搜索框超出1个字符自动清空，添加清空至clickListener
          //that.$searchbox.val('');
        }
      });
  ...
  $(document)
    ...
    .on('keydown' + EVENT_KEY, '.bootstrap-select, .bootstrap-select [role="listbox"], .bootstrap-select .bs-searchbox input', Selectpicker.prototype.keydown)
    .on('focusin.modal', '.bootstrap-select, .bootstrap-select [role="listbox"], .bootstrap-select .bs-searchbox input', function (e) {
      e.stopPropagation();
    });
